...
public class DamageableShapedOreRecipe extends ShapedOreRecipe
{
    ...
    private int[] mirror(int[] array)
    {
        ...
        for (int col = 0; col < width; col++)
        {
            ...
            {
                result[width - 1 - col + row * width] = array[col + row * width];
            }
        }
        ...
        for (int col = 0; col < width; col++)
        {
            for (int row = 0; row < height; row++)
            {
                ...
                if (amount > 0)
                {
                    ...
                    items[invIndex] = stack;
                } else
                {
                    items[invIndex] = ForgeHooks.getContainerItem(inv.getStackInSlot(invIndex));
                }
            }
        }
