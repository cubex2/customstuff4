...
class ShapedRecipe extends SimpleContent
{
    ...
    private void addRecipe()
    {
        ...
        Constructor<DamageableShapedOreRecipe> constructor = cubex2.cs4.util.ReflectionHelper.getConstructor(recipeClass, int[].class, ItemStack.class, Object[].class);
        DamageableShapedOreRecipe recipe = cubex2.cs4.util.ReflectionHelper.newInstance(constructor, createDamageAmounts(), result.getItemStack(), getInputForRecipe());
    ...
    private boolean matchesInput(ShapedOreRecipe recipe)
    {
        ...
        if (width != getRecipeWidth())
            return false;
        if (height != getRecipeHeight())
            return false;
